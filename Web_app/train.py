...
def input_fn(request_body, content_type=TEXT_CONTENT_TYPE):
    logger.info('Deserializing the input data.')
    # process an image uploaded to the endpoint
    if content_type == JPEG_CONTENT_TYPE:
        img = open_image(io.BytesIO(request_body))
        return img
    # process a URL submitted to the endpoint TEXT_CONTENT_TYPE = 'text/plain'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model-arch', type=str, default='resnet18', metavar='MA',
                        help='model arch (default: resnet18)')   
